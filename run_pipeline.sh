#!/bin/bash
echo "--- TRI-NODE PING-PONG PIPELINE INITIATED ---"
echo "Clearing stale processes on 100.66.100.83:7777..."
ssh hal@100.66.100.83 "fuser -k 7777/tcp || true"
echo "[CLEANUP] "
echo ">>> STARTING BUFFER NODE SINDY SERVER (hal@100.66.100.83) <<<"
echo "Server launch: ssh hal@100.66.100.83 cd ~/hxseq-vsgx4/teleparallel_sim_photons && python3 -u sindy_zmq_server.py --port 7777 --no_llm --galactic_spin 0.0"
...
echo "[PIPELINE] Fetching aggregate_states.npz from thejfisher@100.122.147.67..."
echo "Running: scp thejfisher@100.122.147.67:~/AI_Vault/teleparallel_sim_photons/aggregate_states.npz ./aggregate_states.npz"
scp thejfisher@100.122.147.67:~/AI_Vault/teleparallel_sim_photons/aggregate_states.npz "./aggregate_states.npz"
echo "[PIPELINE] Successfully downloaded aggregate_states.npz"
